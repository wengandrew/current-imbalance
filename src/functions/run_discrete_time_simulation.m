function out = run_discrete_time_simulation(t, I, Qa, Qb, Ra, Rb, ...
    za0, zb0, ocv, Vmax)
    ...
    % Truncate results after the cell hits Vmax
    % Nans happen when the OCV function runs out of data. We need to be
    % careful 
    idx = [find(Vt > Vmax) ; find(isnan(Vt))]; 
    za(idx) = [];
    zb(idx) = [];
    Ia(idx) = [];
    Ib(idx) = [];
